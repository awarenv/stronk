...
class PykeysTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock = MagicMock()
        self.mock[0] = pykeys._shuffle_key(self.key)
    ...
    def test_default_key_generation_is_one_key(self):
        self.assertEqual(len(pykeys.generate_random_keys()), 1)

    def test_default_key_generation_key_length_is_16(self):
        self.assertEqual(len(pykeys.generate_random_keys()[0]), 16)

    def test_generate_max_100_keys(self):
        self.assertEqual(len(pykeys.generate_random_keys(100)), 100)

    def test_error_returns_correct_message_if_keys_greater_than_100(self):
        self.assertEqual(pykeys.generate_random_keys(101), "ERROR: Number of keys must be <= 100\n")
    
    def test_error_returns_correct_message_if_keys_is_zero(self):
        self.assertEqual(pykeys.generate_random_keys(
    ...
    def test_all_too_large_errors_can_concatenate(self):
        self.assertEqual(pykeys.generate_random_keys(101, 257),
